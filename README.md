...
как **редактируемые слои** (фреймы, текст, шейпы, вектора-иконки) — по принципу `html.to.design`.

![Архитектура](docs/architecture.svg)

Claude генерирует HTML → вызывает MCP-инструмент → HTML летит в открытый плагин →
плагин рендерит его в изолированном iframe, читает геометрию и стили и строит
нативные слои Figma.

## Скриншоты

<!-- Положи свои PNG из Figma в папку docs/ с такими именами — и они появятся здесь -->
| HTML / артефакт Claude | Результат в Figma |
|---|---|
| ![до](docs/screenshot-source.png) | ![после](docs/screenshot-figma.png) |

> Файлы `docs/screenshot-source.png` и `docs/screenshot-figma.png` добавь сам
> (скриншоты не входят в коммит) — после этого блок выше отрисуется.
...
После установки зависимостей (`cd server && npm install`) и импорта плагина:

- **`start-server.bat`** — двойной клик запускает мост `ws://localhost:3055` для
  **Claude Desktop** или ручного режима. Окно не закрывать.
- **`start-remote.bat`** — запускает HTTP-сервер + ngrok для **claude.ai в браузере**
  и печатает готовый URL для Connectors. Токен берётся из `token.txt` (не в репозитории;
  при первом запуске генерируется автоматически).

> Не запускай оба сразу — они делят порт 3055.
